...
public class Day15Manager : MonoBehaviour
{
    ...
    [SerializeField] private CanvasGroup meetingPlaceBg;
    [SerializeField] private float roomFadeInDuration = 0.6f;
    [SerializeField] private float backgroundTransitionDuration = 0.5f;
    ...
    private void Awake()
    {
        ...
        if (meetingPlaceBg != null)
            meetingPlaceBg.alpha = 0f;
    ...
    private void PlayIntroAnim()
    {
        ...
        // 1) Room background visible from the start
        if (roomBg != null)
        {
            roomBg.alpha = 1f;
            roomBg.gameObject.SetActive(true);
        }

        // 2) Intro black bg starts dimmed on top of room
        if (introBg != null)
        {
        ...
        // 5) Room bg stays at alpha 1; no extra fade needed
    ...
    private void StartMidDialogueAfterPhone()
    {
        ...
        if (roomBg != null)
            seq.Append(roomBg.DOFade(0f, backgroundTransitionDuration).SetEase(Ease.InOutCubic));

        if (meetingPlaceBg != null)
            seq.Join(meetingPlaceBg.DOFade(1f, backgroundTransitionDuration).SetEase(Ease.InOutCubic));
    ...
    private void OnMidDialogueAfterPhoneFinished()
    {
        if (endOfDayReportUI != null)
        {
            endOfDayReportUI.SetSpecialMessage(
                "Fifteen days of choices led to this moment. Alex gave what she could, and faced the outcome with all the courage she had left."
            );
        }
