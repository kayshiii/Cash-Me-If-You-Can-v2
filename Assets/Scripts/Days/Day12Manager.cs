...
public class Day12Manager : MonoBehaviour
{
    ...
    [Header("Background")]
    [SerializeField] private CanvasGroup benchBg;
    [SerializeField] private float benchFadeInDuration = 0.6f;
    [SerializeField] private float delayBeforeBench = 0.1f;

    [Header("Dialogue")]
    [SerializeField] private DialogueController dialogueController;
    [SerializeField] private DialogueData introDialogue;   // before budget system
    [SerializeField] private DialogueData midDialogue;     // Boyet confrontation

    [Header("Phone / System UI")]
    ...
    [SerializeField] private string nextSceneName = "CreditsOrNext"; // set in Inspector
    ...
    private void PlayIntroAnim()
    {
        ...
        // 1) Bench background visible from the start
        if (benchBg != null)
        {
            benchBg.alpha = 1f;
            benchBg.gameObject.SetActive(true);
        }

        // 2) Intro black bg starts dimmed on top of bench
        if (introBg != null)
        {
        ...
        // 5) Bench bg stays at alpha 1; no extra fade needed
    ...
    // ====== PHASE 3: BUDGET GAMEPLAY ======
    private void OpenPhoneForBudget()
    {
    ...
    // Called by SystemScreenController.ProceedExitFlow()
    public void CloseFromSystemScreen()
    {
        if (currentPhase != Phase.BudgetGameplay) return;

        Debug.Log("[Day12] CloseFromSystemScreen called. Proceeding to mid dialogue.", this);
    ...
    // ====== PHASE 4: MID DIALOGUE (Boyet confrontation) ======
