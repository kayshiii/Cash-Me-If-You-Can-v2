...
public class Day13Manager : MonoBehaviour
{
    ...
    [SerializeField] private string nextSceneName = "Day 14";
        ...
        if (endOfDayReportUI != null)
        {
            ...
            if (acceptedEvent)
            {
                endOfDayReportUI.SetSpecialMessage(
                    "You accepted today’s income event and took the extra support that came your way."
                );
            }
            else
            {
                endOfDayReportUI.SetSpecialMessage(
                    "You declined today’s income event and chose not to rely on extra help."
                );
            }
        }
