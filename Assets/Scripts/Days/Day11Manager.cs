...
public class Day11Manager : MonoBehaviour
{
    ...
    [SerializeField] private float schoolFadeInDuration = 0.6f;
    [SerializeField] private float delayBeforeSchool = 0.1f;
    ...
    [SerializeField] private string nextSceneName = "Day 12";
    ...
    private void PlayIntroAnim()
    {
        ...
        // 1) School background visible from the start
        if (schoolBg != null)
        {
            schoolBg.alpha = 1f;
        ...
        // 2) Intro black bg starts dimmed on top of school
        if (introBg != null)
        {
        ...
        if (dayTitle != null)
        {
            ...
            dayTitle.localScale = Vector3.zero;
            seq.Append(dayTitle.DOScale(titleOriginalScale, titlePopDuration).SetEase(Ease.OutBack));
        }
        ...
        // 5) School bg stays at alpha 1; no extra fade needed here
    ...
    private void OnFinalDialogueFinished()
    {
        if (endOfDayReportUI != null)
        {
            endOfDayReportUI.SetSpecialMessage(
                "Alex is exhausted, but she keeps pushing forward because the deadline is close and Boyet still matters to her."
            );
        }
